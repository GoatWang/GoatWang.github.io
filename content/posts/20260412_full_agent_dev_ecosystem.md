...
- testing
- ios
summary: 過去一個月，我真的像是進入另一種開發模式。從 Web 到 iPhone Simulator、從 feature worktree 到獨立驗證環境，我把整套全 Agent 開發流程慢慢補齊。這篇文章整理八個我目前最有感的實戰建議。
cover:
  image: /images/posts/20260412_full_agent_dev_ecosystem/cover.png
---

這一個月來，我使用 Agent 開發的模式真的完全改變了，我不再看任何一行程式碼、我不再審查 Agent 的 diff，我只做需求規劃跟交付驗收，甚至只問問題，讓 Agent 自己驗收。

每天我的日常都是，至少四個 Codex sessions，加上 3 個 Telegram Claude sessions。然後每次打開遠端 Mac Mini 的螢幕，都有自動化流程在 Simulator 上跑，同時我每天可以交付 20 個以上驗證過的 commits。

在這個過程中，我發現跟傳統開發方法上已經有極大的差異，這個發展太快，大概還沒有人寫成 best practice，但影響真的太巨大，工具上、心態上、認知上每個層面都是衝擊。我也不確定我的語言能力是不是好到可以把這個震驚傳遞給大家。

而這些衝擊，讓我可以在一個月內，寫出我以前一年都未必寫得出來的作品。

以下是我一個月踩坑下來，八個目前最有感的建議，也是我目前的 practice，歡迎來跟我交流。
...
如何讓自己可以保持在長時間高強度的工作狀態的關鍵也在這。因為要追蹤三條到四條開發主線，所以每送出一個 prompt 之後，就會有下一個 terminal 有問題等著你去解決，所以就會強迫自己思考每一個 session 的下一步，於是就不會有等待跟無聊的時間。我甚至可以做到一整天15小時以上在做開發，而不感到疲倦，直到我血尿去看醫生。
...
因為每一個 session 都會開發自己的 feature worktree，會有自己的資料夾跟 code logic，都會需要跑驗證，總不能 merge 回去再驗證，所以驗證流程在跑的過程，就要給它們乾淨的環境去跑。例如 web 開發，就要寫好 start server 的 script，然後定義好每一個 session 用哪個 port、資料夾、資料庫去跑驗證，這樣才能真正把平行開發做起來。自從我把這個實踐執行在這個專案之後，開發效率至少快了兩倍。絕對不要小看驗證流程的工作量，好的 test plan 的設計，可能讓 AI 跑驗證工作的時間是開發時間的好幾倍。
...
這些細節，使用者一輩子不會注意到，但你不抄就會自己撞——而且通常會撞好幾次。
